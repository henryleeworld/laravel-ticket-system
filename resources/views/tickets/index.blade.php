    ...
    <div class="mb-4 flex justify-between">
        ...
        <div class="flex space-x-2">
            <select class="block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50" name="status" id="status" onChange="window.location.href=this.value">
                <option value="{{ clearQueryString('status') }}">-- {{ __('SELECT STATUS') }} --</option>
                @foreach(\Coderflex\LaravelTicket\Enums\Status::cases() as $status)
                    <option value="{{ toggle('status', $status->value) }}" @selected($status->value == request('status'))>{{ __($status->name) }}</option>
                @endforeach
            </select>
            ...
            <select class="block w-full rounded-md border-gray-300 shadow-sm focus-within:text-primary-600 focus:border-primary-300 focus:ring-primary-200 focus:ring focus:ring-opacity-50" name="priority" id="priority" onchange="window.location.href=this.value">
                <option value="{{ clearQueryString('priority') }}">-- {{ __('SELECT PRIORITY') }} --</option>
                @foreach(\Coderflex\LaravelTicket\Enums\Priority::cases() as $priority)
                    <option value="{{ toggle('priority', $priority->value) }}" @selected($priority->value == request('priority'))>{{ __($priority->name) }}</option>
                @endforeach
            </select>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="border-b bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                            ...
                            <th class="px-4 py-3">{{ __('Categories') }}</th>
                            <th class="px-4 py-3">{{ __('Labels') }}</th>
                            @hasanyrole('admin|agent')
                                <th class="px-4 py-3">{{ __('Assigned to') }}</th>
                            @endhasanyrole
                            <th></th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y">
                        @forelse($tickets as $ticket)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 space-x-2">
                                    ...
                                    @role('admin')
                                        <form action="{{ route('tickets.destroy', $ticket) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure?') }}')" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
                                            <x-primary-button>
                                                {{ __('Delete') }}
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-3" colspan="4">{{ __('No tickets found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
