                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b bg-gray-50">
                            ...
                            <th class="px-4 py-3">{{ __('Is visible') }}</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y">
                        @forelse($categories as $category)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    {{ $category->is_visible ? __('Yes') : __('No') }}
                                </td>
                                <td class="px-4 py-3 space-x-2">
                                    ...
                                    <form action="{{ route('categories.destroy', $category) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure?') }}')" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <x-primary-button>
                                            {{ __('Delete') }}
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-3" colspan="4">{{ __('No categories found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
