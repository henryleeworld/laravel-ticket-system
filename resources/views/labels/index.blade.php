            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y">
                        @forelse($labels as $label)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 space-x-2">
                                    ...
                                    <form action="{{ route('labels.destroy', $label) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure?') }}')" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <x-primary-button>
                                            {{ __('Delete') }}
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-3" colspan="4">{{ __('No labels found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
